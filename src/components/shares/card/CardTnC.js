...
import React from "react";
import { Card, CardBody, CardFooter, Checkbox, Button } from "@nextui-org/react";

export function CardTnC() {
  return (
    <Card className="max-w-[1100px] shadow-lg">
      <CardBody className="px-5 py-2 text-medium text-default-400 mb-4 overflow-y-auto h-[400px]">
        <p className="text-sub-text regular-18 pt-4 text-justify">
        <p>1. Introduction</p>
...
<p>These terms and conditions are governed by and construed in accordance with the laws of Indonesia, without regard to its conflict of law principles.</p>
      ...
      <CardFooter className="gap-3 mt-4 flex items-center"> {/* Use flexbox for alignment */}
        <div className="flex items-center gap-2"> {/* Nest another div for better control */}
          <Checkbox
            defaultSelected={false}
            color="primary"
            style={{
        ...
        <div className="flex justify-between mt-4">
          <Button type="small-secondary" title="Decline" />
          <div className="ml-1">
            <Button type="small" title="Accept" />
          </div>
        </div>
      </CardFooter>
    </Card>
